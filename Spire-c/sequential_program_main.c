...
/*regurding the case in which more than one thread process the same file*/
thread_file thread_files[PROCESSORS_NUMBER];
int external_sources[PROCESSORS_NUMBER]; //in case a thread becomes the one sharing and not closing the file
int is_sharing_stream[PROCESSORS_NUMBER] = {0};  //indicates when a thread identified by "identity_in_arrays" index must share
int is_processing_sequentially[PROCESSORS_NUMBER] = {1};
/**/
...
void process_next_read(char** header, char** genom, int current_header_size, int current_genom_size, int identity_in_arrays);
void share(int occupied_thread_index, int free_threads[PROCESSORS_NUMBER], int dim);
...
*/
  ...
  for(i = 0; i < PROCESSORS_NUMBER; i++) {
    ...
    *j = i;
    ...
    thread_files[i].mutex = malloc(sizeof(pthread_mutex_t));
    pthread_mutex_init(thread_files[i].mutex, NULL);
  }
  /**/
...
*/
  int threads_have_done = 0; 

  while(!threads_have_done) {

   int first_thread_occupied = -1;
   int available_threads_found[PROCESSORS_NUMBER];
   int dim = 0;
   int available_cont = 0;

   for (i = 0; i < PROCESSORS_NUMBER; i++) {
     if (available_threads[i]) {
       //printf("found %d\n");
       available_cont++;  //to check if all the threads are available
       if (first_thread_occupied != -1) {  //if you have already found one
         share(i, available_threads_found, dim);
         dim = 0;
       }
       else {
         available_threads_found[dim] = i;
         dim++;
       }
     }
     else {   //if not available found
       /*If no available thread found until now and the current thread is the one to share the file to the others*/
       if ((first_thread_occupied = -1) && !(files_to_process[i].deleted)) {
         first_thread_occupied = i;
         if (dim > 0)    //if some thread is doing nothing
           share(i, available_threads_found, dim);
         dim = 0;
       }
     }
    // printf("cont: %d\n", available_cont);
     if (available_cont >= PROCESSORS_NUMBER - 1)
        threads_have_done = 1;  
   }
  }
/*
...
void process_next_read(char** header, char** genom, int current_header_size, int current_genom_size, int identity_in_arrays) {
  ...
  fprintf(thread_files[identity_in_arrays].factorization_file, "%s\n%s\n", *header, result1);
  result2 = create_fingerprint(result1, thread_files[identity_in_arrays].kfingerprint_file, *header);
  fprintf(thread_files[identity_in_arrays].fingerprint_file, "%s %s\n", *header, result2);
  fprintf(thread_files[identity_in_arrays].oneformat_file, "%s %c %s %c %s\n", *header, '$', result2, '$', result1);
  //free? 
...
/*
  It assignes all given available threads to the occupied thread. If the occupied thread is in "processing sequentially" state
  it waits it to be in "sharing processing" state.
  pre-condition occupied_thread_index:
                  available_threads[occupied_thread_index] = 1 &&
                  !files_to_process[*identity_in_arrays].deleted;
  pre-condition dim: 0<=dim<PROCESSORS_NUMBER;
  pre-condition available_threads: for each x in available_threads:
                    available_threads[x] == 1 &&
                    !files_to_process[x].deleted &&
                    is_sharing_stream[x] == 0 &&
                    thread_files[x] == NULL;
  post-condition:
    dim == 0 && available_threads[x] == 0 && is_sharing_stream[x] == 0 && thread_files[x] == thread_files[occupied_thread_index]
    && is_processing_sequentially[occupied_thread_index] == 1 && is_sharing_stream[occupied_thread_index] == 1;
*/
void share(int occupied_thread_index, int free_threads[PROCESSORS_NUMBER], int dim) {

  if ((occupied_thread_index < 0) || (occupied_thread_index > PROCESSORS_NUMBER)) {
    printf("invalid value for occupied_thread_index in share function: %d\n", occupied_thread_index);
    pthread_exit(1);
  }

   if (dim > PROCESSORS_NUMBER) {
    printf("invalid value for dim (number of available threads) in share function: %d\n", dim);
    pthread_exit(1);
  }

  if (is_processing_sequentially[occupied_thread_index] && (dim > 0)) {
      /*This is the lock when the thread is already processing a file and has to go to the "share processing" state.
      To avoid critic region problem creator process waits for the thread to recive the command and stop the processing*/
      is_sharing_stream[occupied_thread_index] = 1;  //give the command to share
      while(is_processing_sequentially[occupied_thread_index]);
  }
  while (dim-- > 0) {
   external_sources[dim] = occupied_thread_index;
   is_sharing_stream[free_threads[dim]] = 1;
  }
}
...
//  int current_header_size_beckup;
  ...
  while(1) {  
    if (is_sharing_stream[*identity_in_arrays]) {
        printf("%d in sharing\n", *identity_in_arrays);
  //    printf("entering first block\n");
      if (!files_to_process[*identity_in_arrays].deleted && cont != 0) {
        cont--;
        goto sequential_state;
      }
      int identity = (!files_to_process[*identity_in_arrays].deleted) ? *identity_in_arrays : external_sources[*identity_in_arrays];
      if (files_to_process[*identity_in_arrays].deleted)
        thread_files[identity].thread_dedicated++;
      available_threads[*identity_in_arrays] = 0;
      pthread_mutex_lock(thread_files[identity].mutex);
      /* critic region: reading from file with safe_fgets*/
      if (thread_files[identity].fasta_file == NULL)
        goto after_processing;
      while ( (header = safe_fgets(header, &current_header_size, thread_files[identity].fasta_file)) != NULL ) {
        genom = safe_fgets(genom, &current_genom_size, thread_files[identity].fasta_file);
      /**/ 
        if (slow_thread)
          sleep(1);
        pthread_mutex_unlock(thread_files[identity].mutex);
 //       printf("unlocked %d\n", *identity_in_arrays);
        process_next_read(&header, &genom, current_header_size, current_genom_size, identity);
  //      printf("lock request %d\n", *identity_in_arrays);
        pthread_mutex_lock(thread_files[identity].mutex);
      }
after_processing: pthread_mutex_unlock(thread_files[identity].mutex);
 //     printf("unlocked %d\n", *identity_in_arrays);
 //     printf("after execution\n");
      if (!files_to_process[*identity_in_arrays].deleted) {
 //      printf("I am %d\n", *identity_in_arrays);
  //      printf("%d\n", thread_files[identity].thread_dedicated);
        while((thread_files[identity].thread_dedicated) > 1);  //wait other threads to finish
        thread_files[identity].deleted = 1;
  //      printf("proceeding closing\n");

        fclose(thread_files[identity].fasta_file);
        fclose(thread_files[identity].factorization_file);
        fclose(thread_files[identity].fingerprint_file);
        fclose(thread_files[identity].kfingerprint_file);
   //     printf("one format is probably the problem\n");
        fclose(thread_files[identity].oneformat_file);
        printf("%s processed\n", files_to_process[*identity_in_arrays].filename);
...
sequential_state: thread_files[*identity_in_arrays].thread_dedicated = 1;
        ...
        thread_files[*identity_in_arrays].deleted = 0;
        thread_files[*identity_in_arrays].factorization_file = open_towrite_file("factorization", filename, directory_path);
        thread_files[*identity_in_arrays].fingerprint_file = open_towrite_file("fingerprint", filename, directory_path);
        thread_files[*identity_in_arrays].kfingerprint_file = open_towrite_file("kfingerprint", filename, directory_path);
        thread_files[*identity_in_arrays].oneformat_file = open_towrite_file("oneformat", filename, directory_path);
        thread_files[*identity_in_arrays].thread_dedicated = 1;
        if ((thread_files[*identity_in_arrays].fasta_file = fopen(path, "r")) == NULL) {
          printf("error: %s\n\n", strerror(errno));
        }
        if (thread_files[*identity_in_arrays].fasta_file != NULL) {
          while ( (header = safe_fgets(header, &current_header_size, thread_files[*identity_in_arrays].fasta_file)) != NULL ) {
            genom = safe_fgets(genom, &current_genom_size, thread_files[*identity_in_arrays].fasta_file);
            process_next_read(&header, &genom, current_header_size, current_genom_size, *identity_in_arrays);
            if (slow_thread)
              sleep(1);
            if (is_sharing_stream[*identity_in_arrays]) {
              is_processing_sequentially[*identity_in_arrays] = 0;
              goto end;
            }
          }
          fclose(thread_files[*identity_in_arrays].kfingerprint_file);
          fclose(thread_files[*identity_in_arrays].factorization_file);
          fclose(thread_files[*identity_in_arrays].fingerprint_file);
          fclose(thread_files[*identity_in_arrays].oneformat_file);
          files_to_process[*identity_in_arrays].deleted = 1;
          thread_files[*identity_in_arrays].deleted = 1;
          printf("%s processed\n", path);
...
void test_process_file_create_file_to_process(char* path_test) {
...
void test_process_file_check_result() {

  char header_test[300];
  char genom[600];
  char s[300], c1, c2;
  int f1, f2, f3, f4, z1, z2, z3,z4, cont;
  FILE *test_file;
...
void test_process_file() {
  ...
  test_process_file_create_file_to_process(path_test);
  ...
  test_process_file_check_result();
 ...
 if(fgets(s, 300, test_file) != NULL)
  ...
  printf("test conditions: processing fasta with 3 reads in shared mode with only one thread\n");

  test_process_file_create_file_to_process(path_test);

  /*initialization that the thread would have performed in sequantial mode*/
  available_threads[0] = 0;
  char *directory_path = path_test;
  char filename[300];
  ...
  thread_files[0].deleted = 0;
  thread_files[0].factorization_file = open_towrite_file("factorization", filename, directory_path);
  thread_files[0].fingerprint_file = open_towrite_file("fingerprint", filename, directory_path);
  thread_files[0].kfingerprint_file = open_towrite_file("kfingerprint", filename, directory_path);
  thread_files[0].oneformat_file = open_towrite_file("oneformat", filename, directory_path);
  thread_files[0].thread_dedicated = 1;
  thread_files[0].mutex = &mutex;
  if ((thread_files[0].fasta_file = fopen(path, "r")) == NULL) {
    printf("error: %s\n\n", strerror(errno));
  }
  ...
  end_of_processing = 0;
  files_to_process[0].deleted = 0;
  is_sharing_stream[0] = 1;
  is_processing_sequentially[0] = 0;
  pthread_mutex_init(thread_files[0].mutex, NULL);
  pthread_attr_init(&att);
  pthread_create(&tid, &att, process_file, &index);
  ...
  pthread_mutex_destroy(thread_files[0].mutex);  //to check for corretness
  test_process_file_check_result();
  
  printf("\ntest_case_passed\n\n");

  printf("test conditions: processing fasta with 3 reads in sequential mode at first an than in shared mode\n");
  ...
  files_to_process[0].deleted = 0;
  end_of_processing = 0;
  is_sharing_stream[0] = 0;
  is_processing_sequentially[0] = 1;
  slow_thread = 1;
  available_threads[0] = 0;
  thread_files[0].mutex = &mutex;
  pthread_mutex_init(thread_files[0].mutex, NULL);
  ...
  printf("test conditions: processing fasta with 3 reads with two threads: the first one will start in sequential mode ");
  printf("and than the second one will join the processing, so they start coordinating\n");
  ...
  files_to_process[0].deleted = 0;
  files_to_process[1].deleted = 1;
  
  end_of_processing = 0;
  is_sharing_stream[0] = 0;
  is_sharing_stream[1] = 0;

  is_processing_sequentially[0] = 1;
  is_processing_sequentially[1] = 0;
  
  slow_thread = 1;
  available_threads[0] = 1;
  available_threads[1] = 1;

  thread_files[0].mutex = &mutex;
  thread_files[1].mutex = thread_files[0].mutex;
  ...
  pthread_create(&tid, &att, process_file, &index);
  pthread_create(&tid2, &att, process_file, &index2);

  external_sources[1] = 0;
  is_sharing_stream[0] = 1;
  while(is_processing_sequentially[0]);
  is_sharing_stream[1] = 1;
  strcat(path_test, "/");
  strcat(path_test, "test_file.fasta");
  communicate_max_fact_length(0);
  fact_choice = 1;
  ...
  printf("check for correctess directly\n");
  getchar();
  remove("test_file-factorization");
  remove("test_file-fingerprint");
  remove("test_file-kfingerprint");
  ...
  printf("processing test_file passed\n");
